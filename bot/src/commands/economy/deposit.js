...
class DepositCommand extends Command {
  ...
  async run(interaction) {
    ...
    if (amount > user.wallet) {
      return interaction.reply({
        embeds: [SDFEmbed.error("Insufficient wallet balance")],
        ephemeral: true,
      })
    }

    try {
      await this.client.economyManager.deposit(interaction.user.id, interaction.guild?.id || "global", amount)
      ...
      const embed = new SDFEmbed()
        ...
        .setDescription(`\`\`\`ansi
\u001b[0;32m╔══════════════════════════════╗
║  Deposited: ${Helpers.formatNumber(amount).padEnd(16)}║
╠══════════════════════════════╣
