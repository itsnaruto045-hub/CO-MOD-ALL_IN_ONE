...
export default {
  ...
  async execute(interaction, client) {
    ...
    const cooldown = await checkCooldown(odbc, "monthly")
    if (!cooldown.ready) {
    ...
    const reward = randomInt(25000, 50000)

    await client.economy.addMoney(odbc, reward, "wallet", "monthly")
    await setCooldown(odbc, "monthly")
